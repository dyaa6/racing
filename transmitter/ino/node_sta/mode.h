...
const char mode_page[] PROGMEM = R"rawliteral(
  <!DOCTYPE html>
<html lang='en'>
<head>
...
const changeState=()=>{
  ...
  if(switch1.checked){
    document.getElementById('current_mode').innerHTML="HTTP mode";
  }
  else{
    document.getElementById('current_mode').innerHTML="NRF mode";
  }
}
  ...
  var currentHost=window.location.hostname;
    try {
        ...
        new Promise((resolve, reject) => {
          ...
          const url = 'http://'+currentHost+'/setmode?mode=true';
          xhr.onreadystatechange = () => {
            if (xhr.readyState === 4) {
              if (xhr.status === 200) {
                ...
                document.getElementById('msg').innerHTML=('لايوجد اتصال ');
          ...
          new Promise((resolve, reject) => {
            const xhr = new XMLHttpRequest();
            const url = 'http://'+currentHost+'/setmode?mode=false';
            xhr.onreadystatechange = () => {
              if (xhr.readyState === 4) {
                if (xhr.status === 200) {
                    if(xhr.responseText=='Timer enabled'){
